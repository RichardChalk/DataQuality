...
namespace DataQuality.Services
{
    public class ReferentialIntegrity
    {
        ...
        {
            // 1. Skapa en snabb-lista (HashSet) med alla CustomerId som faktiskt finns i kundlistan
            ...
            {
                // 2. Kontrollera om orderns CustomerId finns i vår lista över giltiga kunder
                if (!validCustomerIds.Contains(order.CustomerId))
                {
                    // Om ID:t saknas är detta en "föräldralös" (orphaned) order
                    Console.WriteLine($"VARNING: Order {order.OrderId} refererar till CustomerId {order.CustomerId} som INTE existerar!");
                    orphanCount++;
                }
            }
